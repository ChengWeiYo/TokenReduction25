...
def parse_args():
    ...
    parser.add_argument('--keep_model', nargs='+', type=str, default=None,
                        help='Subset of models (backbones) to keep')
    ...
    parser.add_argument('--separate_by_backbone', action='store_true',
                        help='If set, plot one figure per backbone model')
...
def preprocess_df(args):
    ...
    if args.keep_model is not None and 'pt' in df.columns:
        df = df[df['pt'].isin(args.keep_model)]
    print(len(df))
